...
def plot_result(result, imgfp, timgfp):
    ...
    for i in bboxes:
        ...
        copytImg = timg[int(x1):int(x2), int(y1-10):int(y2-10)].copy()
        ...
        print(f'{path} face_{str(cnt).zfill(3)} temperature: {round(temp, 2)}℃')
...
def pred():
    ...
    imgfp = f"{input_path}/photo.jpg"
    timgfp = f"{input_path}/msx.jpg"

    data = dict(img_info=dict(filename=imgfp), img_prefix=None)
    ...
    plot_result(result, imgfp, timgfp)
